...
let CrossChainBridge = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec4 = view(), _dec(_class = (_class2 = class CrossChainBridge {
  ...
  bridgeTransfer({
    ...
    const proposalStatus = this.checkProposalStatus({
      proposalId
    });
...
}, _applyDecoratedDescriptor(_class2.prototype, "bridgeTransfer", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "bridgeTransfer"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "checkProposalStatus", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "checkProposalStatus"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getBridgeStatus", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "getBridgeStatus"), _class2.prototype), _class2)) || _class);
function getBridgeStatus() {
  const _state = CrossChainBridge._getState();
  if (!_state && CrossChainBridge._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = CrossChainBridge._create();
  if (_state) {
    CrossChainBridge._reconstruct(_contract, _state);
  }
  const _args = CrossChainBridge._getArgs();
  const _result = _contract.getBridgeStatus(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CrossChainBridge._serialize(_result, true));
}
function checkProposalStatus() {
  ...
  const _args = CrossChainBridge._getArgs();
  const _result = _contract.checkProposalStatus(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CrossChainBridge._serialize(_result, true));
}
function bridgeTransfer() {
  ...
  const _args = CrossChainBridge._getArgs();
  const _result = _contract.bridgeTransfer(_args);
  CrossChainBridge._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CrossChainBridge._serialize(_result, true));
}

export { bridgeTransfer, checkProposalStatus, getBridgeStatus };
//# sourceMappingURL=cross_chain_bridge.js.map
