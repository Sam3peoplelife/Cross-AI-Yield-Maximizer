...
let RiskManager = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = view(), _dec(_class = (_class2 = class RiskManager {
...
}, _applyDecoratedDescriptor(_class2.prototype, "setUserRiskProfile", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "setUserRiskProfile"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getUserRiskProfile", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "getUserRiskProfile"), _class2.prototype), _class2)) || _class);
function getUserRiskProfile() {
  const _state = RiskManager._getState();
  if (!_state && RiskManager._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = RiskManager._create();
  if (_state) {
    RiskManager._reconstruct(_contract, _state);
  }
  const _args = RiskManager._getArgs();
  const _result = _contract.getUserRiskProfile(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(RiskManager._serialize(_result, true));
}
function setUserRiskProfile() {
  ...
  const _args = RiskManager._getArgs();
  const _result = _contract.setUserRiskProfile(_args);
  RiskManager._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(RiskManager._serialize(_result, true));
}

export { getUserRiskProfile, setUserRiskProfile };
